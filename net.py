...
class DDNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, device_model in enumerate(self.device_models):
            ...
            size = (B, 16, 14, 9)                         # size of the array or shape of the noise
            noise = torch.normal(mu[i], std_dev[i], size) # create a tensor with gausian noises

            h, prediction = device_model(x[:, i])         # h is the device exit data, dimentions :[32, 16, 24, 9]
                                                          # prediction is the prediction per device, dimentions: [32, 10]
            h = h + noise                                 # adding the noise to the device output data
            hs.append(h)
            predictions.append(prediction)

        # TO ADD GAUSIAN NOISE IF SIGMA IS CONST PER DEVICE
